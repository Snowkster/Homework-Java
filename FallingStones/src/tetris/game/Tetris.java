...
public class Tetris extends JPanel implements Runnable {
            ...
            public void keyPressed(KeyEvent e) {
                ...
                switch (e.getKeyCode()) {
                    ...
                    case KeyEvent.VK_LEFT:
                        if (canMove(fallingShape, Posicion.left))
                            move(Posicion.left);
                        break;
 
                    case KeyEvent.VK_RIGHT:
                        if (canMove(fallingShape, Posicion.right))
                            move(Posicion.right);
                        break;
                    ...
                    case KeyEvent.VK_DOWN:
                        if (!fastDown) {
                            fastDown = true;
                            while (canMove(fallingShape, Posicion.down)) {
                                move(Posicion.down);
                                repaint();
                            }
                            shapeHasLanded();
    ...
    public void run() {
        ...
        while (Thread.currentThread() == fallingThread) {
            ...
            if (!scoreboard.isGameOver()) {
                if (canMove(fallingShape, Posicion.down)) {
                    move(Posicion.down);
                } else {
                    shapeHasLanded();
                }
    ...
    void move(Posicion pos) {
        fallingShapeRow += pos.y;
        fallingShapeCol += pos.x;
    }
 
    boolean canMove(Shape s, Posicion pos) {
        for (int[] p : s.pos) {
            int newCol = fallingShapeCol + pos.x + p[0];
            int newRow = fallingShapeRow + pos.y + p[1];
            if (grid[newRow][newCol] != EMPTY)
                return false;
        }
        return true;
